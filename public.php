...
foreach ($stats as $pStats) {
    ...
    if (ISSET($pStats['monsterHP'])) {
        $monster = new Monster(rand($pStats['monsterHP']['min'], $pStats['monsterHP']['max']), rand($pStats['monsterStrength']['min'],
                                $pStats['monsterStrength']['max']), rand($pStats['monsterDefence']['min'], $pStats['monsterDefence']['max']),
                                rand($pStats['monsterSpeed']['min'], $pStats['monsterSpeed']['max']), rand($pStats['monsterLuck']['min'], $pStats['monsterLuck']['max']));
    }
}
